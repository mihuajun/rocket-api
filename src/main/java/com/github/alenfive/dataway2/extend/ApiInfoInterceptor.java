...
public abstract class ApiInfoInterceptor implements HandlerInterceptor {
    ...
    private ApiInfo getApiInfo(HttpServletRequest request){
        ...
        return mappingFactory.getPathList().stream().filter(item->pattern.equals(item.getPath()) && (method.equals(item.getMethod()) || "ALL".equals(item.getMethod()))).findFirst().orElse(null);
    }
